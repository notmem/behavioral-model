...
SimpleSwitch::SimpleSwitch(int max_port)
  ...
  add_required_field("intrinsic_metadata", "priority");
}
...
void SimpleSwitch::enqueue(int egress_port, std::unique_ptr<Packet> &&packet) {
    ...
    int priority = phv->get_field("intrinsic_metadata.priority").get_int();

    egress_buffers[egress_port].push(priority, std::move(packet));
}
...
void SimpleSwitch::egress_thread(int port) {
  ...
  while(1) {
    std::unique_ptr<Packet> packet;
    egress_buffers[port].pop(&packet);
