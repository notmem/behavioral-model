...
struct QPacket {
  ...
  QPacket(int priority, std::unique_ptr<Packet> pkt)
    : priority(priority), pkt(std::move(pkt)) { }
...
struct QPacketComp {
  bool operator()(const QPacket &lhs, const QPacket &rhs) const {
    return lhs.priority < rhs.priority;
  }
};

class PacketQueue : public PriorityQueue<QPacket, QPacketComp>
{
private:
...
public:
  PacketQueue()
    : PriorityQueue<QPacket, QPacketComp>(1024, WriteReturn, ReadBlock),
  ...
  void pop(std::unique_ptr<Packet> *pkt) {
    QPacket qpkt;
    PriorityQueue<QPacket, QPacketComp>::pop(&qpkt);
    *pkt = std::move(qpkt.pkt);
    // enforce rate
  ...
  void push(int priority, std::unique_ptr<Packet> &&pkt) {
    QPacket qpkt(priority, std::move(pkt));
    PriorityQueue<QPacket, QPacketComp>::push(std::move(qpkt));
  }
